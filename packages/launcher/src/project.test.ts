...
test("resolveSession derives a stable URL-safe id and persists the mapping", () => {
  ...
  const id = resolveSession(fs, "/home/.belay", "/Users/kevin/dev/belay", "2026-06-26T00:00:00Z");
  assert.match(id, /^[a-z0-9-]+$/);
  assert.equal(id.includes("/"), false);

  // The mapping is persisted and reused verbatim on the next resolve (same project → same session).
  const map = loadProjectMap(fs, "/home/.belay");
  assert.equal(map["/Users/kevin/dev/belay"]?.sessionId, id);
  const again = resolveSession(
    fs,
    "/home/.belay",
    "/Users/kevin/dev/belay",
...
test("distinct project roots get distinct persisted sessions", () => {
  const fs = fakeLauncherFs();
  const a = resolveSession(fs, "/home/.belay", "/work/app-a", "t");
  const b = resolveSession(fs, "/home/.belay", "/work/app-b", "t");
  assert.notEqual(a, b);
  const map = loadProjectMap(fs, "/home/.belay");
...
test("a malformed projects.json reads as an empty map (never throws)", () => {
  const fs = fakeLauncherFs();
  fs.writeFile("/home/.belay/projects.json", "not json{");
  assert.deepEqual(loadProjectMap(fs, "/home/.belay"), {});
});
