...
import { createBelayClient, SdkError } from "../src/index";
...
describe("createBelayClient over a real session-store (M2)", () => {
  it("ensures a session, publishes an event, and replays it through readLog", async () => {
    const client = createBelayClient({ sessionUrl: store.url });
  ...
  it("reports an unreachable backend as a typed SdkError naming the operation", async () => {
    ...
    const client = createBelayClient({ sessionUrl: "http://127.0.0.1:1" });
