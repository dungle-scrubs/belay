...
A progress report exists at `.plans/continuation-handoff/progress-report.md`. It lists every current-cutoff behavior for every milestone as a checkbox.

Mandatory rules for agents working on this plan:

1. Before starting a milestone, run `mise x node@22 -- npx tsx ~/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "continuation-handoff"` and read that milestone in the progress report.
...
- The `ask-user-tool` plan owns the generic approval question surface; handoff adapts to it.
...
**Gate from previous:** Generated slash flow is green, and `ask-user-tool` approval adapter is available.
...
- **Dependencies:** M5, ask-user-tool M7
- **Effort:** M
- **Tasks:**
...
2. If `ask_user` is not ready, slash commands can proceed, but model proposal approval remains blocked and this plan does not reach complete.
...
```bash
mise x node@22 -- npx tsx ~/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "continuation-handoff"
```
...
Canonical decisions live in `.plans/continuation-handoff/plan.db`. Query them with:

```bash
mise x node@22 -- npx tsx ~/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "continuation-handoff"
```
