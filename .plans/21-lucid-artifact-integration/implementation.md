...
- [ ] `20-artifact-panel-ux` - Lucid should render inside Trevor's reusable artifact panel rather than opening a separate Lucid browser workflow.
...
| Constraint | Impact |
|---|---|
| Artifact panel first | Lucid is a viewer/feedback mode inside `20-artifact-panel-ux`, not a separate primary UI. |
...
**Gate from previous:** `20-artifact-panel-ux` viewer registry exists.

#### M1: Artifact Kind and Metadata

- **Dependencies:** `20-artifact-panel-ux`
- **Effort:** M
- **Tasks:**
...
- **Dependencies:** M1-M6, `20-artifact-panel-ux`
- **Effort:** M
- **Tasks:**
...
The progress report is `.plans/21-lucid-artifact-integration/progress-report.md`. It tracks Lucid as a consumer of the reusable artifact panel. It does not track the generic artifact panel UX itself.

Before implementation resumes, run:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "21-lucid-artifact-integration"
```
...
Canonical decisions are in `.plans/21-lucid-artifact-integration/plan.db`. Query them with:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "21-lucid-artifact-integration"
```
