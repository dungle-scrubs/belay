...
- Live open follow-up (D-065 provider auth/catalog + full model chooser): 62 features, 37 completed, 25 remaining (M1 contract + M2 chooser surface + M5 no-secret auth UI built). Migration is LIVE end-to-end (M6): `user.message` carries a `model` ModelRef, the web stamps it on every turn via the threaded send path, and the host resolves through `resolveUserTurnModel` (legacy provider/reasoning still decode). The header first-prompt title is in place. The selection-state foundation landed: `apps/web/src/model-selection.ts` projects the announced provider roster into the chooser's source/catalog contract via the migration bridges (no host work needed first), and `useModelSelection` owns persisted ModelPreferences (active/recent/pinned/reasoning) with the active model falling back to today's provider until an explicit pick. Both pure-tested. REMAINING is the App WIRING (a visible change needing a live/Storybook review): swap the `ModelSelector` in `panel-controls.tsx` for `SplitModelControl` (quick-picker → `selection.select`), open `ModelChooser` as a takeover of the PanelHost transcript/composer center column (sidebars remain), sync the legacy provider/reasoning on select, and feed `selection.active` to the send path. Then M4 richer host read models (gateway catalogs, real auth freshness) + catalog-browse UI + M7 verification.
