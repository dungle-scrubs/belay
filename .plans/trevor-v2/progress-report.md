...
- Live open follow-up (D-065 provider auth/catalog + full model chooser): 62 features, 37 completed, 25 remaining (M1 contract + M2 chooser surface + M5 no-secret auth UI built). Migration is LIVE end-to-end (M6): `user.message` carries a `model` ModelRef, the web stamps it on every turn via the threaded send path, and the host resolves through `resolveUserTurnModel` (legacy provider/reasoning still decode). The header first-prompt title is in place. Remaining is the chooser UI + its data: M4 host source/catalog read models (the pure `queryCatalog` exists) → M3 App split-control wiring (SplitModelControl + ModelChooser already built as components) → M6 ModelPreferences-as-source-of-truth selection state → M4/M7 catalog browsing UI + verification. These are interdependent (the split control's full-chooser region needs the host source/catalog read models).
