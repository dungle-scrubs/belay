...
- [~] Support filters for source, provider/family, configured-only, tools, vision, reasoning, local/cloud, context size, recent, pinned, and recommended (tools/vision/reasoning + search + **recent + pinned** live - the chooser now has Recent/Pinned chips + a per-row pin star wired to `togglePin`/`ModelPreferences`, verified live; `filterCatalog` already supports family/kind/minContext under the hood; configured-only/local-cloud/context-size chips + recommended remain)
