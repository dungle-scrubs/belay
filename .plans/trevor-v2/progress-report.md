...
- [~] User turn events move toward `{ sourceId, modelId, reasoning }` while preserving legacy provider compatibility during migration (protocol + host landed: `user.message` carries an optional `model` ModelRef alongside the legacy provider/reasoning, and the host resolves the turn through `resolveUserTurnModel`; the web SEND wiring - producing the ModelRef from the active selection - rides the M3/M6 web slice)
...
- Live open follow-up (D-065 provider auth/catalog + full model chooser): 62 features, 35 completed, 1 partial, 26 remaining (M1 contract + M2 chooser surface + M5 no-secret auth UI built). Migration foundation landed (M6): `user.message` carries an optional `model` ModelRef and the host resolves the turn through `resolveUserTurnModel` (legacy provider/reasoning still decode). Remaining: M3 App split-control wiring (quick-picker component already built), M4 catalog browsing UI, the web SEND-side selection wiring + reasoning, and M7 verification.
