...
- [ ] `08-tool-detail-takeover` - required before promoted jobs can open the full live detail takeover surface; the first runtime promotion slice can still define process read models, but final UI completion depends on this.
...
- `08-tool-detail-takeover` owns the full detail takeover; this plan supplies the promoted-process read model and entry points.
...
- **Dependencies:** M7, `08-tool-detail-takeover`
- **Effort:** M
- **Tasks:**
...
3. **If detail takeover is not ready:** ship the activity panel with job rows and basic kill controls, but keep full live output in a disabled detail affordance until `08-tool-detail-takeover` lands.
...
The progress report is `.plans/09-shell-promote-background-jobs/progress-report.md`. It tracks shell promotion, promoted process snapshots, the combined task/background support panel, and promoted-job detail integration. It does not track general tool-detail takeover implementation except as a dependency.
...
Canonical decisions are in `.plans/09-shell-promote-background-jobs/plan.db`.
