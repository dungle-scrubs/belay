...
**Gate from previous:** `03-nested-command-menu` and D-075 discovery registry current cutoff are available.
...
- **Dependencies:** M1, M2, `03-nested-command-menu`, D-075
- **Effort:** L
- **Tasks:**
...
Canonical decisions are in `.plans/14-capability-manifest-and-trevor-expert/plan.db`. Query them with:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "14-capability-manifest-and-trevor-expert"
```
