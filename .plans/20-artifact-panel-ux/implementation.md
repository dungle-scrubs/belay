...
3. **If HTML viewer hardening is not ready:** ship non-HTML viewers first and leave Lucid gated behind `21-lucid-artifact-integration`.

## 5. Progress Report Accounting

The progress report is `.plans/20-artifact-panel-ux/progress-report.md`. It tracks only the reusable artifact panel UX, not Lucid-specific annotation/review behavior.

Before implementation resumes, run:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "20-artifact-panel-ux"
```
...
Canonical decisions are in `.plans/20-artifact-panel-ux/plan.db`. Query them with:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "20-artifact-panel-ux"
```
