...
- **Accepted/deferred follow-up:** 7 (Phase 5, gated on `.plans/43`)
...
- [ ] REFACTOR: reuse `28-tool-detail-takeover` primitives.
...
### Phase 5: Model-authored JS (gated on `.plans/43-tool-script`)

Deferred until `43` ships its out-of-process child runner + OS sandbox and that boundary is extracted
...
- [ ] M9 REFACTOR: `43` consumes the shared runner (no behavior change).
