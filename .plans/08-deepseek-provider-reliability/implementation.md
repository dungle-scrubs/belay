...
The progress report for this plan is `.plans/08-deepseek-provider-reliability/progress-report.md`. It treats only current-cutoff checkboxes as blockers. Deferred follow-up is excluded from current blockers. Before implementation resumes, run:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "08-deepseek-provider-reliability"
```
...
Canonical decisions live in `.plans/08-deepseek-provider-reliability/plan.db`.
