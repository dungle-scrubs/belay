...
- [ ] `.plans/01-managed-worktree-hardening` - the cwd-path advisory lock. **Soft dependency, not a
  blocker:** the serial run can operate without it, but 01 is what makes a Trevor-owned mutating
  worktree safe while the user does manual work on `main` concurrently - which is the whole motivation
  for this feature. <!-- D-004 -->

This plan is the **serial precursor of `.plans/46-worktree-fleet`** <!-- D-001 -->. It owns the same
per-plan worktree lifecycle (create -> implement -> commit -> merge-back -> delete) but runs it
**strictly one tree at a time**, with **no** dependency on `.plans/21-workflows-runtime` (the Effect
engine), `.plans/12-bounded-child-takeover` (parallel leaves), or `.plans/15-forkable-sessions-lineage`
(the run is a fresh session, not a fork). `46` later generalizes this same lifecycle to N>1 in parallel.
...
user request (their session)
  ...
  -> [serial driver loop]  for each plan, strictly one at a time:
       1 create + enter a managed worktree                 (/worktree-new + /worktree-switch; 01 cwd lock)
...
This intentionally differs from `46`'s default disposition ("leave branches, human merges"): the serial
...
| No `21`/`12`/`15` dependency | The serial driver stands alone on the shipped engine + handoff-flow + planner implement-mode. |
...
- The per-plan lifecycle is factored as **one reusable unit** so `46-worktree-fleet` can later call it
  per worker leaf - the serial driver is the seam `46` parallelizes. <!-- D-005 -->
...
- **Tasks:**
  1. RED: Cover create + enter of a managed worktree for a queued plan via the existing
     `WorktreeManager` (`/worktree-new` + `/worktree-switch`), acquiring the 01 cwd lock.
...
- **Tasks:**
  ...
  5. REFACTOR: Factor the per-plan lifecycle as a single reusable unit that `46-worktree-fleet` can call
...
- [ ] The driver reuses the shipped `WorktreeManager` + `/worktree-*` commands + `handoff-flow` +
      `planner` implement-mode, with no new git plumbing and no dependency on `21`/`12`/`15`.
...
- Parallel / concurrent worktrees - that is `.plans/46-worktree-fleet` on `.plans/21-workflows-runtime`.
- The Effect workflow runtime (`.plans/21-workflows-runtime`).
- Bounded-child fan-out (`.plans/12-bounded-child-takeover`) and per-tree verifier/audit leaves (`.plans/45-subagent-variants`).
- A richer disposition policy (review queue, per-tree approval UI) beyond auto-merge-on-green /
  stop-and-surface - `46` owns that.
- Session forking / lineage (`.plans/15-forkable-sessions-lineage`) - the serial run is a fresh durable session, not a fork.

---

## Decisions

Canonical decisions are in `.plans/02-serial-worktree-implement/plan.db`. Query with:

```bash
mise x node@22 -- npx tsx ~/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "02-serial-worktree-implement"
```
