...
- [ ] `25-keyboard-shortcuts` - frontmost-surface focus guards and Escape ownership must be settled so detail-view Escape returns to chat without leaking to background surfaces.
- [ ] `27-compact-transcript-layout` - compact rows provide a consistent entry point and detail eligibility language for dense transcript items.
- [x] Existing model chooser transcript-takeover pattern from D-065.
- [x] Existing archive-browser plan pattern in `.plans/26-archive-browser-and-delete` - management/detail surfaces replace the transcript and use a top-left back arrow.
...
- **Dependencies:** M2, `25-keyboard-shortcuts`
- **Effort:** M
- **Tasks:**
...
2. **If Escape routing is not ready:** keep detail view Storybook-only or back-arrow-only until `25-keyboard-shortcuts` lands.
...
The progress report is `.plans/28-tool-detail-takeover/progress-report.md`. It tracks only the transcript-item detail takeover surface. It does not track compact row rendering except as an entry-point dependency, and it does not track artifact-panel/Lucid document viewing.
...
Canonical decisions are in `.plans/28-tool-detail-takeover/plan.db`.
