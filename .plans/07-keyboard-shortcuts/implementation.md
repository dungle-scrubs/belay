...
- [ ] `06-vim-motions-ui` - shortcut routing must coordinate with Vim insert/normal/visual key ownership.
...
- [x] `03-nested-command-menu` - reusable nested command-menu/row chooser patterns exist for command-like surfaces.
...
- **Dependencies:** M1-M2, `03-nested-command-menu`
- **Effort:** M
- **Tasks:**
...
- **Dependencies:** M3, `06-vim-motions-ui`, `03-filesystem-root-taxonomy`
- **Effort:** M
- **Tasks:**
  ...
  5. REFACTOR: Keep config read/write logic shared with `06-vim-motions-ui`.
...
3. **If config persistence is not ready:** ship read-only palette shell first and keep the Vim toggle disabled until `06-vim-motions-ui` config plumbing lands.

## 5. Progress Report Accounting

The progress report is `.plans/07-keyboard-shortcuts/progress-report.md`. It tracks only keyboard shortcut routing, the command palette shell, the persisted Vim toggle command, and the first binding set. It does not implement broader settings, global navigation, or non-keyboard command surfaces.

Before implementation resumes, run:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts check-progress --plan "07-keyboard-shortcuts"
```
...
Canonical decisions are in `.plans/07-keyboard-shortcuts/plan.db`. Query them with:

```bash
mise x node@22 -- npx tsx /Users/kevin/dev/dotfiles/agents/.agents/skills/planner/scripts/plan-db.ts query-decisions --plan "07-keyboard-shortcuts"
```
