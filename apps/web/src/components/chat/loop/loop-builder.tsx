...
import type { CommandParseResult } from "@/commands/command-family";
 ...
 * create a valid loop.
 *
 * Purely presentational and driven entirely by a `CommandParseResult` - the
 * caller parses on every keystroke. There is no Confirm button: submitting the
 * composer (Enter) is the confirmation.
 */
export function LoopBuilder(props: { parse: CommandParseResult; className?: string }) {
  const { parse, className } = props;
  const errors = parse.diagnostics.filter((diagnostic) => diagnostic.severity === "error");

  return (
    <div className={cn("flex flex-col gap-2", className)}>
      <dl className="flex flex-col gap-1">
        {parse.fields.map((row) => (
      ...
      <div className="border-t border-border pt-2">
        <span
          className={cn(
            ...
            parse.ready ? "text-smui-green" : "text-muted-foreground",
          )}
        >
          {parse.ready ? (
