...
function DiffViewerStats({
  ...
  return (
    <span data-slot="diff-viewer-stats" className="flex gap-2 text-xs">
      <span className="text-smui-green">+{additions}</span>
      <span className="text-smui-red">-{deletions}</span>
    </span>
  );
}
    ...
    >
      ...
      <span className="flex-1">
        {oldName && newName && oldName !== newName ? (
          <>
            <span className="text-smui-red">{oldName}</span>
            {" → "}
            <span className="text-smui-green">{newName}</span>
          </>
        ) : (
          displayName
        )}
