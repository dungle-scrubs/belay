...
test("renders recents and picking one launches that root directly", () => {
  ...
  assert.ok(screen.getByText("trevorV2"), "the recent's folder name renders");
  assert.ok(screen.getByText("~/dev/opchain"), "the recent's full root renders");
  fireEvent.click(screen.getByText("trevorV2"));
  assert.deepEqual((props.onPickRecent as ReturnType<typeof vi.fn>).mock.calls, [
    ["~/dev/trevorV2"],
  ]);
});
