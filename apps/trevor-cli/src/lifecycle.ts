import { selectSessions } from "@trevor/sdk";
import { relativeTime, type SessionSummary } from "@trevor/session";

/**
 * The `trevor` session-lifecycle subcommands (D-094 M3). The PURE selection/resolution logic
 * (`selectSessions`, `resolveOpenTarget`, `expandHome`) now lives in `@trevor/sdk` as the single source
...
/** Runs `trevor archive|unarchive <session>`, publishing the durable marker through the SDK. */
