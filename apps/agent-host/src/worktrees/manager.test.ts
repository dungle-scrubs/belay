  ...
  return (cwd) => (args) => {
    ...
    const table: Record<string, { status: number; stdout: string }> = {
      ...
      "rev-parse --path-format=absolute --git-common-dir": {
        status: 0,
        stdout: "/dev/belay/.git\n",
      },
      "rev-parse --show-toplevel": { status: 0, stdout: "/dev/belay\n" },
      ...
      "worktree add -b feat/x /home/.belay/.worktrees/": { status: 0, stdout: "" },
    };
...
const ctx = {
  ...
  baseRepoName: "belay",
  basePath: "/dev/belay",
  baselineSessionId: "belay-base",
  currentPath: "/dev/belay",
};
...
test("create surfaces a git failure as a typed error and records nothing", () => {
  ...
  const failing = gitFactory({
    "/dev/belay": { "worktree add -b feat/x ": { status: 128, stdout: "" } },
  });
...
test("summaries lists a baseline row first, then managed worktrees with git state", () => {
  ...
  assert.equal(rows[0]?.path, "~/belay");
