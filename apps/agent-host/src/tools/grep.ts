...
import { walkWorkspace } from "./search";
  ...
  execute: (args, ops) =>
    Effect.gen(function* () {
      ...
      try {
        ...
        return yield* ops.reject("invalid regular expression");
      }

      const fileGlob = args.glob;

      return yield* ops.attempt(async () => {
        const results: string[] = [];
        let scanned = 0;

        for await (const entry of walkWorkspace(fileGlob)) {
