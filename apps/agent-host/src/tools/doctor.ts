...
/**
 * The `doctor` model-facing tool (D-073 M6): runs Belay's OWN host self-diagnostic and returns the
 ...
 * the model to call it only when the user asks about Belay's own health/setup or why a turn failed,
 ...
 *
 * Responsible for: the doctor tool - rendering Belay's own host health report for the model.
 */
...
export const doctorTool = simpleTool({
  name: "doctor",
  description:
    "Run Belay's own /doctor host self-diagnostic and return a health report covering providers / " +
    ...
    "only when the user asks about Belay's own health, setup, provider/model/tool availability, " +
