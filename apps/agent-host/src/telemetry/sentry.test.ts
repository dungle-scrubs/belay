...
test("Sentry is NOT initialized under test/CI even when a DSN is set", () => {
  const { api, inits } = recordingApi();
  assert.equal(
    bootstrapNodeSentry(api, { NODE_ENV: "test", BELAY_SENTRY_DSN: "https://a@x/1" }),
    false,
  );
  assert.equal(bootstrapNodeSentry(api, { CI: "1", BELAY_SENTRY_DSN: "https://a@x/1" }), false);
...
test("with a DSN, Sentry initializes errors-only with the scrubbing beforeSend", () => {
  const { api, inits } = recordingApi();
  assert.equal(
    bootstrapNodeSentry(api, { NODE_ENV: "production", BELAY_SENTRY_DSN: "https://a@x/1" }),
    true,
  );
