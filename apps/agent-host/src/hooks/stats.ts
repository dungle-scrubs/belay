import { hookApprovalKey } from "./approval";
 ...
 * (`<source>:<id>`), matching how the rest of the runtime identifies a hook.
 *
 * Responsible for: accumulating per-hook run/slow/timeout/failure counters and the Doctor snapshot.
 * Not for: deriving outcomes (./results) or emitting events (M9).
 */
...
export interface HookStats {
  /** Accounts one completed execution and its derived outcome. */
  readonly record: (
    hook: Pick<HookDefinition, "source" | "id" | "timeoutMs">,
...
export function createHookStats(): HookStats {
  ...
  return {
    record: (hook, execution, outcome) => {
      ...
      if (
        !execution.timedOut &&
        execution.durationMs > SLOW_HOOK_THRESHOLD_RATIO * hook.timeoutMs
      ) {
        entry.slowRuns += 1;
      }
